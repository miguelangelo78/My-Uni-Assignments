...
uint16_t TM_USART_printf(USART_TypeDef* USARTx, const char fmt[], ...) {
	...
	va_end(args);
	TM_USART_Puts(USARTx, printf_buff);
	return ret;
}
...
void TM_USART_Send(USART_TypeDef* USARTx, uint8_t* DataArray, uint8_t count) {
	uint8_t i;
	TM_USART_t* u = TM_USART_INT_GetUsart(USARTx);
	/* If we are not initialized */
	if (u->Initialized == 0) {
