...
/************************************/
#define BAUD_TO_PRESCALER(baudrate) (uint32_t)(POLLFREQ / (baudrate))
...
void suart_rx_poll(suart_t * chan) {
	...
	if(chan->rxing) {
		...
		default: {
				uint8_t bit = R_GPIO_PinRead(chan->rxpin);
...
void suart_tx_poll(suart_t * chan) {
	...
	if(chan->txing) {
		...
		case 0: /* Send start bit */
			R_GPIO_PinWrite(chan->txpin, GPIO_LEVEL_LOW);
			chan->tx_delta_polls++;
			break;
		case 9: /* Send stop bit and finish transmission */
			R_GPIO_PinWrite(chan->txpin, GPIO_LEVEL_HIGH);
		...
		default: /* Send 'nth' data bit */
			R_GPIO_PinWrite(chan->txpin, (chan->tx_fifo->buff[chan->tx_fifo->buff_fifo_idx] & (1 << (chan->tx_delta_polls - 1))) ? GPIO_LEVEL_HIGH : GPIO_LEVEL_LOW);
			chan->tx_delta_polls++;
			break;
		}
	}
...
void suart_init_all_channels(void) {
	for(uint8_t i = 0; i < SUART_MAX_CHANNEL_COUNT; i++) {
		...
		suart_channels[i].rxpin             = (gpio_port_pin_t)0;
		suart_channels[i].txpin             = (gpio_port_pin_t)0;
#if SUART_RX_AUTOFLUSH_ENABLE == 1
...
suart_t * suart_init(uint32_t baudrate, gpio_port_pin_t rxpin, gpio_port_pin_t txpin, suart_rx_cback_t rx_cback_ptr, uint32_t tx_bufflen, uint32_t rx_bufflen) {
	...
	if(tx_bufflen > 0 || rx_bufflen > 0) {
		...
		suart_channels[suart_alloc_channel].rxpin          = rxpin;
		suart_channels[suart_alloc_channel].txpin          = txpin;
		...
		if(txpin) {
			R_GPIO_PinDirectionSet(txpin, GPIO_DIRECTION_OUTPUT);
			R_GPIO_PinWrite(txpin, GPIO_LEVEL_HIGH);
		}
		...
		spawn_task_args("suart_rx_cbk_dspchr", suart_rx_callback_dispatcher, channel_ptr);
