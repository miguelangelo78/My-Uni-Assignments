...
typedef enum mtu_clk_edges_e
{
    ...
    MTU_CLK_ANY_EDGE     = 0x10,  // See hardware manual for exceptions
} mtu_clk_edges_t;

/* The possible counter clearing source selections. Index into register settings table.
 * Do not change the order of this enum list! */
...
typedef enum mtu_actions_e
{
    ...
    MTU_ACTION_ONESHOT   = 0x08,      // Perform the timer cycle and actions one time only (cannot be specified with MTU_ACTION_REPEAT)
    MTU_ACTION_REPEAT    = 0x10,      // Continuously repeat the timer cycle and actions (cannot be specified with MTU_ACTION_ONESHOT)
