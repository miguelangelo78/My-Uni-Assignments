...
#define MTU_RX_VERSION_MINOR           (10)
...
#elif defined BSP_MCU_RX111 || defined BSP_MCU_RX110 || defined BSP_MCU_RX113 || defined BSP_MCU_RX210 || defined BSP_MCU_RX231
/* ICU.IER.IEN  bits for each TGI source.   0xFF = not available this channel.      */
/*                  { TGIA      TGIB      TGIC          TGID         }  */
...
extern const uint8_t g_chnl_clear_src[MTU_CHANNEL_MAX][MTU_CLR_MAX];
...
extern bool mtu_calc_clock_divisor(uint8_t chan, uint8_t *div_idx, uint32_t frq_target, mtu_clk_edges_t clock_edge);
extern uint16_t mtu_calc_tgr_ticks(uint16_t pclk_div, uint32_t frq_target, mtu_clk_edges_t clock_edge);
