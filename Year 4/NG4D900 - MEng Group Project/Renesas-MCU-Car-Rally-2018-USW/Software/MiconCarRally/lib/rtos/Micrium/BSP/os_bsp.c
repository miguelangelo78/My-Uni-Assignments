...
void  OS_BSP_TickInit (void)
{
    ...
    IPR(CMT0, CMI0) = 3;                                        /* Set interrupt priority.                              */
...
#if OS_CFG_TICK_FAST_TICK_EN

fast_tick_cback_t ftick_cback = 0;
bool ftick_as_interrupt       = false;

void OS_FastTickRegister(fast_tick_cback_t fast_tick_task, bool as_interrupt) {
	ftick_cback        = fast_tick_task;
	ftick_as_interrupt = as_interrupt;
}
...
#if OS_CFG_TICK_FAST_TICK_EN
	if(ftick_cback && ftick_as_interrupt)
		ftick_cback();
#endif
