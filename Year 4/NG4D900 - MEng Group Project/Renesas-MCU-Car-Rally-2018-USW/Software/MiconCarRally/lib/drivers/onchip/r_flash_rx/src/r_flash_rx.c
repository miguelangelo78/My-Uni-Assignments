...
void R_FlashCodeCopy(void)
{
    ...
    /* Initialize pointers */
    p_ram_section = (uint8_t *)__sectop("RPBTLDRFRAM");
    p_rom_section = (uint8_t *)__sectop("PBTLDRFRAM");

    /* Copy code from ROM to RAM. */
    for (bytes_copied = 0; bytes_copied < __secsize("PBTLDRFRAM"); bytes_copied++)
    {
    ...
    /* Initialize pointers */
    p_ram_section = (uint8_t *)__sectop("RPBTLDRFRAM2");
    p_rom_section = (uint8_t *)__sectop("PBTLDRFRAM2");

    /* Copy code from ROM to RAM. */
    for (bytes_copied = 0; bytes_copied < __secsize("PBTLDRFRAM2"); bytes_copied++)
    {
        p_ram_section[bytes_copied] = p_rom_section[bytes_copied];
    }
#endif
