...
enum SERVO_RETCODE servo_ctrl(servo_t * handle, float angle) {
	...
	/* Update PWM value */
	spwm_set_duty(handle->dev_handle, mapfloat(angle, SERVO_MIN_ANGLE, SERVO_MAX_ANGLE, SERVO_MIN_ANGLE_DUTY, SERVO_MAX_ANGLE_DUTY));
...
enum SERVO_RETCODE servo_sweep(servo_t * handle, float min_angle, float max_angle, uint32_t delay, float increment, bool from_cur_angle) {
	...
	if(from_cur_angle) {
		for(float angle = handle->angle; angle >= min_angle; angle -= increment) {
			servo_ctrl(handle, angle);
			rtos_delay(delay);
		}
	}

	for(float angle = min_angle; angle < max_angle; angle += increment) {
		servo_ctrl(handle, angle);
		rtos_delay(delay);
	}
